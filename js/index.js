    ...
    product_list.innerHTML += `
      <div class="col-6 col-sm-4 col-lg-3 mt-3 p-3 product">
        <div class="product-img">
          <img src="img/product/product.jpeg" alt="${product.name}">
        </div>
        ...
        <div class="text-end">
          <button onclick="add_to_cart(this.id)" id="${product.id}" class="btn btn-success btn-sm">Add to Cart
          </button>
        </div>
      </div>
    `;
...
function login_success() {
  ...
  <button class="btn ms-3 disabled text-light">${username}</button>
...
function show_cart_list(products) {
  ...
  <tr>
   ...
   <th>${total}</th>
   <th>
     <button class="btn btn-success btn-sm">Order</button>
   </th>
</tr>
`;
  ...
  db.transaction(function (tx) {
    ...
    tx.executeSql(
      ...
      function (tx, result) {
        ...
        var message = `Delete "${product_name}" successfully.`;
